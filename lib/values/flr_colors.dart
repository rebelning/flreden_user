import 'package:flutter_eden/eden.dart';

class FLRColors {
  FLRColors._();
  static Color white = const Color(0xffFFFFFF);
  static Color white40 = const Color(0xffFFFFFF).withOpacity(0.4);

  static Color black = const Color(0xff000000);
  static Color black05 = const Color(0xff000000).withOpacity(0.05);
  static Color black10 = const Color(0xff000000).withOpacity(0.10);
  static Color black20 = const Color(0xff000000).withOpacity(0.20);
  static Color black60 = const Color(0xff000000).withOpacity(0.60);
  static Color black70 = const Color(0xff000000).withOpacity(0.70);
  static Color black04 = const Color(0xff000000).withOpacity(0.04);
  static Color colorFEFEFE = const Color(0xffFEFEFE);
  static Color color252C32 = const Color(0xff252C32);
  static Color color252C3250 = const Color(0xff252C32).withOpacity(0.5);

  static Color color5AAAFC = const Color(0xff5AAAFC);
  static Color color3792F0 = const Color(0xff3792f0);
  static Color colorC7E4FF = const Color(0xffC7E4FF);
  static Color color98CBFF = const Color(0xff98CBFF);
  static Color colorDEEEFF = const Color(0xffDEEEFF);
  static Color color63A7F2 = const Color(0xff63A7F2);
}
